...
| A-20 | Per-dimension R² distribution for GenePT regression (PI ask from prior review) | — | **done 2026-05-16** | New `scripts/per_dim_r2.py` loads saved probe weights, computes per-dim R² across 1,536 dims for 5 headline cells. Headline DNABERT-2 \texttt{meanG} CDS: 1,527/1,536 dims have R²>0, median 0.190, top-429-dims (28%) carry 50% summed R². Signal is broadly distributed, not concentrated. Two-panel supplementary figure landed in submodule `paper/figures/per_dim_r2_distribution.png`; results.tex §3.2 + appendix.tex updated; submodule HEAD now `28bcb04`. |
| C-21 | Verify Li 2026 SeqCLIP citation date | — | **done 2026-05-16** | arXiv `2602.12286` (Yanan Li et al.) is real (v2 2026-05-08, NeurIPS 2026 under review), but title differs and the paper never uses the name "SeqCLIP". Updated bib title + added timeline note; rephrased intro from "SeqCLIP and related ... methods" to "shared-tokenizer and other ... methods" to match the cited paper's actual contribution. |
| R-1 | Methods §Dataset "why ~3k genes" rationale + §Limitations scope-(b) sentence | — | **done 2026-05-16** | `methods.tex:5` extended with within-five-families ceiling (99.9 % saturation = 3,244/3,247) and the reasons not to expand to the full 18,836 GenePT-covered PC genes. `discussion.tex:19` §Limitations gets the matching scope-(b) sentence cross-referencing Methods §Dataset. |
| R-2 | Intro + abstract: narrow the biological question per reviewer | — | **done 2026-05-16** | `introduction.tex:5-7` and `abstract.tex:1` rewritten with reviewer's two-part formulation: (i) curated functional annotation (5-way family) and (ii) free-text natural-language descriptions (GenePT). "Lightweight frozen-encoder and linear-probe" framing preserved. |
| R-3 | Methods §TSS: biological motivation for non-coding context | — | **done 2026-05-16** | `methods.tex:40` prepended one paragraph explaining co-regulated module / promoter motif rationale (NF-κB near immune-response genes, ISRE elements near interferon-stimulated genes) and naming the two TSS-arm hypotheses. Checked the bootstrap numbers: the F1-vs-R² retention asymmetry the reviewer hypothesised is *not* supported by the data (F1 retention 61 % vs R² retention 53 % on TSS), so the methods sentence sets up the hypothesis-test framing without overclaiming. |
| R-4 | Move A3 (confusion) + A4 (UMAP) from appendix to main results §3.1 | — | **done 2026-05-16** | `results.tex` §3.1 now has the confusion-matrix figure inline; the family-coloured UMAP becomes the left panel of the new two-panel Figure in §3.5 (so it is still in main). Labels renamed `fig:s-confusion`→`fig:confusion`, `fig:s-umap`→`fig:umap-overlay`; appendix.tex `\subsection{Supporting Visual Diagnostics}` keeps only the per-dim R² figure. |
| R-5 | Figure 1 schematic redesign | — | not started | Reviewer ask: example gene → chunking → pooling → probe outputs. Existing fig1.png is functional but text-heavy. Lowest priority of R-1..R-6. |
| R-6a | Pfam fetch for all 3,244 genes | — | **done 2026-05-16** | New `scripts/fetch_pfam_annotations.py` uses MyGene.info batch POST (~30 s for 3,244 genes). Output: `data/gene_pfam.parquet`. Coverage 98.5 % Pfam + 99.7 % InterPro. Per-family Pfam coverage 98–100 %. |
| R-6b | Canonical-Pfam map per family | — | **done 2026-05-16** | New `scripts/canonical_pfam_map.py`: curated map of textbook canonical domains per family (TFs: zf-C2H2, homeobox, bHLH, forkhead, HMG-box, etc.; kinases: Pkinase, PK_Tyr; GPCRs: 7tm_1–4, Frizzled; ion: Ion_trans, BK, IRK, cNMP; immune: Lectin_C, Ig, NACHT, LRR, V-set). Coverage 63 % (TF) to 97 % (GPCR). |
| R-6c | Render UMAP with Pfam overlay | — | **done 2026-05-16** | New `scripts/render_umap_with_pfam_overlay.py`: two-panel figure (left family-coloured, right canonical-domain-coloured with family-as-marker-shape). Reuses existing UMAP params (n_neighbors=15, min_dist=0.1, random_state=42). Outputs `paper/figures/umap_with_pfam_overlay.png` and persists coords to `data/umap_nt_v2_meanD_coords.parquet` for downstream analysis. |
| R-6d | Quantify motif interpretation: per-family Fisher's exact + bootstrap OR | — | **done 2026-05-16** | New `analysis/motif_interpretation.py`. Mixed-cluster = UMAP 5-NN family purity < 0.6. **TF: OR 4.19 [3.30, 5.45], p = 3.4e-32** (non-canonical TFs 37.4 % mixed vs canonical 12.5 %). GPCR: OR 33.4 [6.6, 95.9], p = 9.1e-5. Pooled: OR 2.04, p = 2.5e-18. Ion/immune/kinase weaker due to higher base-rate mixed-cluster fractions. Output: `data/motif_interpretation_summary.json`. **Reviewer's "NAIL the interpretation" hypothesis confirmed for TFs and GPCRs.** |
| R-6e | Paper integration of motif analysis | — | **done 2026-05-16** | New §3.5 "Canonical-domain structure explains UMAP separation" in `results.tex` with the two-panel UMAP figure and the Fisher's exact result. New paragraph "What the encoder is picking up" in `discussion.tex`. The §3.1 paragraph now cross-references §3.5 for the boundary-gene story. |
| R-7 | Rebuild PDF, push submodule, bump parent (pending compile) | — | not started | Needs pdflatex (not available on this machine). Final step: `pdflatex paper/main.tex` ×2, verify refs/cites resolve, commit submodule on `revision/tss-multi-encoder`, push, bump parent submodule pointer, refresh parent `dna_to_text.pdf`. |
