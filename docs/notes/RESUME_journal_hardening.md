...
## Pending (3 tasks) — exact next steps
- **#4a GTF overlap** (CPU, independent): build `src/tss_overlap/overlap.py` — for each 196,608 bp TSS window compute fraction overlapping target CDS/exon/UTR/intron, neighbor exon, intergenic, using `data/annotation/Homo_sapiens.GRCh38.110.gtf.gz` and `enformer_windows.centered_window`. TSS gene coords come from the cached `_lookup/*.json` (under the TSS window cache dir). Output a fraction table + figure.
