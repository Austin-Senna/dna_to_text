...
**Two-panel figure** (`analysis/viz/figures/umap_dnabert2_tokenisation_compare.png`): DNABERT-2 UMAP under `regular_mean` vs `special_mean`.
...
> **Reproduce:** `uv run python analysis/demo/cross_modal.py` — full per-gene retrieved summaries in `analysis/demo/output.md`.
...
- **Unsupervised UMAP** (`analysis/viz/figures/umap_nt_v2_meanD.png`). Only TF and GPCR form clean clusters; the three smaller families overlap in 2D. The linear probe still recovers them (κ 0.82) — UMAP optimises local density, not the linear directions the probe uses.
