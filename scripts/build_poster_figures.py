...
Run: uv run scripts/build_poster_figures.py
"""
...
def _chance(ax):
    """Draw the chance floor; return a legend handle (a text label on the line
    collides with short bars' value labels)."""
    ax.axhline(FLOOR, color=_CHANCE, ls="--", lw=1.5)
    return Line2D([0], [0], color=_CHANCE, ls="--", lw=1.5,
                  label=f"chance ({FLOOR:.3f})")
...
def fig_comparator_f1():
    ...
    _legend(ax, [Patch(facecolor=PC_COMP, label="composition baseline"),
                 ...
                 Patch(facecolor=PC_ESM, hatch="//", label="ESM-2 (protein LM, ceiling)"), ch],
            "upper left")
    fig.tight_layout()
...
def fig_comparator_r2():
    ...
    ax.bar(x, vals, color=colors, edgecolor="white", linewidth=1.6,
           hatch=["//" if c[2] == C_ESM else "" for c in CELLS])
    _grid(ax)
    ax.set_ylim(0, 0.25)  # R^2 scale, not F1; headroom for the legend
    ax.set_yticks(np.arange(0, 0.201, 0.05))
    _bar_labels(ax, x, vals, fmt="{:.3f}", dy=0.003, fs=17)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=35, ha="right")
    ax.set_ylabel("Ridge-to-GenePT $R^2$\n(higher is better)")
    _legend(ax, [Patch(facecolor=PC_COMP, label="composition baseline"),
                 Patch(facecolor=PC_DNA, label="frozen DNA encoder"),
                 Patch(facecolor=PC_ESM, hatch="//", label="ESM-2 (protein LM, ceiling)")],
            "upper left")
    fig.tight_layout()
    fig.savefig(OUT / "comparator_r2.pdf")
    plt.close(fig)
    print("comparator_r2", dict(zip(labels, [round(v, 3) for v in vals])))


# Reader-facing pooling names (same mapping as the paper's pool_names.POOL_DISPLAY).
POOL_NAME = {"meanmean": "Mean", "specialmean": "Mean +\nBoundary",
             "maxmean": "Max", "clsmean": "Mean-CLS", "meanD": "Ends + Mean",
             "meanG": "Ends + Mean\n+ Max"}
...
def fig_pooling_heatmap():
    ...
    plt.close(fig)
    print("pooling_heatmap", {e: [round(v, 3) for v in row] for e, row in zip(ENCODERS, vals)})
...
def fig_split_bars():
    ...
    _legend(ax, [Patch(facecolor="#777", alpha=0.5, label="random split (leaks paralogs)"),
                 Patch(facecolor="#777", label="homology-aware split"), ch],
            "upper left")
    fig.tight_layout()
